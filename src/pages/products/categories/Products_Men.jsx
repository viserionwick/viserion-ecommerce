...
import { addDoc, updateDoc, collection, setDoc, doc } from 'firebase/firestore';
...
const PRODUCTS_MEN = () => {
    ...
    useEffect(() => {
      let fetchLimit = 25;
      if(category){
        fetchProducts(fetchLimit, "subCategories", category, true, ["unisex", "men"]);
      } else {
        fetchProducts(fetchLimit, undefined, undefined, true, ["unisex", "men"]);
      }
    }, [category]);
    ...
    // Tab Menu
    const { data: tabMenuLinks, loading: tabMenuLinksLoading } = useFetch("categories", "men");
  ...
  return (
    <>
    <TABMENU title={"MEN"} links={tabMenu} loading={tabMenuLinksLoading}/>
