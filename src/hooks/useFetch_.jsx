...
const useFetch_ = (isFunc = true, fetchLimit = 0, collectionName, documentName, fieldName = false, fieldValue, isFeildArray = false, category = null, isSearch = false, isRealTime = false) => {
  ...
  useEffect(() => {
    if (!isFunc) {
      fetchProducts(fetchLimit, collectionName, documentName, fieldName, fieldValue, isFeildArray, category, isSearch, isRealTime)
    }
    
  }, [ collectionName, documentName, fieldName, fieldValue, isFeildArray, isSearch, isRealTime ])
  
  const fetchProducts = (fetchLimit = 0, collectionName, documentName, fieldName = false, fieldValue, isFeildArray = false, category = null, isSearch = false, isRealTime = false) => {
    ...
    const queryCalc = (lastVisibleFetch) => {
      if(!lastVisibleFetch) {
        if( !fieldName && !fieldValue ){
          if (category) {
            let q = query(collRef, where("category", "in", category), orderBy("createdAt", "desc"), limit(fetchLimit));
