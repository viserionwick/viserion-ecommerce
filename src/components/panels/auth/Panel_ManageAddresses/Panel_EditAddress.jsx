...
const PANEL_EDITADDRESS = ({addressIndex}) => {
  ...
  useEffect(() => {
    if (userData) {
      setFormData({
        type: userData.addressBook[addressIndex].type,
        firstName: userData.addressBook[addressIndex].firstName,
        lastName: userData.addressBook[addressIndex].lastName,
        addressLine_1: userData.addressBook[addressIndex].addressLine_1,
        addressLine_2: userData.addressBook[addressIndex].addressLine_2,
        country: userData.addressBook[addressIndex].country,
        defaultAddress: userData.addressBook[addressIndex].defaultAddress ? true : false
      })
    } else {
      setFormData({
        type: addressBook_guest[addressIndex].type,
        firstName: addressBook_guest[addressIndex].firstName,
        lastName: addressBook_guest[addressIndex].lastName,
        addressLine_1: addressBook_guest[addressIndex].addressLine_1,
        addressLine_2: addressBook_guest[addressIndex].addressLine_2,
        country: addressBook_guest[addressIndex].country,
        defaultAddress: addressBook_guest[addressIndex].defaultAddress ? true : false
    })
    }
  }, [userData]);
    ...
    if (!empty) {
      ...
      if (formData.defaultAddress) {
        if (userData ? userData.addressBook[addressIndex].defaultAddress : addressBook_guest[addressIndex].defaultAddress) {
            ...
            newAddressBook = [newFormData, ...newAddressBook.filter(address => !address.defaultAddress)];
            ...
            let oldDefaultAddress = newAddressBook.filter(address => address.defaultAddress)[0];
            const {defaultAddress, ...oldDefaultAddressToNormal} = oldDefaultAddress;
            let removeOldDefaultAddress = newAddressBook.filter(address => !address.defaultAddress);
            newAddressBook = removeOldDefaultAddress;
            newAddressBook = [oldDefaultAddressToNormal, ...newAddressBook];

            // remove old address
            newAddressBook.splice(addressIndex, 1);
            
            // replace to top position
        ...
        if (userData ? userData.addressBook[addressIndex].defaultAddress : addressBook_guest[addressIndex].defaultAddress) {
          ...
          delete newFormData.defaultAddress;

          newFormData.fullName = fullName;

          let oldDefaultAddress = newFormData;
          let newDefaultAddress = newAddressBook[1];
          newDefaultAddress.defaultAddress = true;

          newAddressBook = newAddressBook.slice(2);
          newAddressBook = [newDefaultAddress, oldDefaultAddress, ...newAddressBook];
          ...
          handleClose(); // this was default, now normal
        }
        else {
          let newAddressBook = userData ? [...userData.addressBook] : [...addressBook_guest];
          let newFormData = {...formData};
          ...
          newAddressBook.splice(addressIndex, 1, newFormData);
...
export default PANEL_EDITADDRESS